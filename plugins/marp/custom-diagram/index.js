...
function parseCustomDiagramBlock(content) {
  ...
  const radialTeam = type === "radial-team" ? parseRadialTeam(relevantLines) : null;
  return {
    type,
    title: radialTeam ? radialTeam.title : "",
    members: radialTeam ? radialTeam.members : [],
  };
}

function parseRadialTeam(lines) {
...
function renderSvg(data) {
  if (data.type === "radial-team") return renderRadialTeamSvg(data.title || "Team", data.members || []);

  return renderUnknownSvg(data.type);
}

function renderRadialTeamSvg(title, members) {
...
function radialTeamLayout(members) {
  ...
  const topMembers = members.slice(0, topCount).map((member, index) => [member, index]);
  const bottomMembers = members.slice(topCount).map((member, index) => [member, topCount + index]);
  ...
  for (const [rowMembers, y, centerAnchorY, isTopRow] of rows) {
    ...
    rowMembers.forEach(([member, originalIndex], index) => {
...
customDiagramMarkdownItPlugin.connectorGeometry = connectorGeometry;
